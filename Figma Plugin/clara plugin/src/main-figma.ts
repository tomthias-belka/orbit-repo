...
function getTokenType(variable: Variable): string {
  console.log(`[getTokenType] Processing variable: ${variable.name}, type: ${variable.resolvedType}, scopes:`, variable.scopes);

  // Handle no scopes or empty array
  if (!variable.scopes || !Array.isArray(variable.scopes) || variable.scopes.length === 0) {
    console.log(`[getTokenType] No scopes provided, using generic type mapping`);
  ...
  if (variable.scopes.includes('ALL_SCOPES')) {
    console.log(`[getTokenType] ALL_SCOPES detected, using generic mapping`);
    return FIGMA_TO_TOKEN_TYPE[variable.resolvedType as keyof typeof FIGMA_TO_TOKEN_TYPE] || 'string';
  }
  ...
  if (variable.scopes.length === 1) {
    ...
    if (mappedType) {
      console.log(`[getTokenType] Mapped single scope "${scope}" to type "${mappedType}"`);
      return mappedType;
    } else {
      console.log(`[getTokenType] No mapping found for scope "${scope}", using generic mapping`);
      return FIGMA_TO_TOKEN_TYPE[variable.resolvedType as keyof typeof FIGMA_TO_TOKEN_TYPE] || 'string';
    }
  }

  // Handle multiple scopes - use priority order
  console.log(`[getTokenType] Multiple scopes detected (${variable.scopes.length}), applying priority logic`);
  ...
  for (const priorityScope of scopePriority) {
    if (variable.scopes.includes(priorityScope as any) && SCOPE_TO_TYPE_MAPPING[priorityScope as keyof typeof SCOPE_TO_TYPE_MAPPING]) {
      ...
      console.log(`[getTokenType] Found priority scope "${priorityScope}" -> "${mappedType}"`);
      return mappedType;
    }
  }
  ...
  for (const scope of variable.scopes) {
    const mappedType = SCOPE_TO_TYPE_MAPPING[scope as keyof typeof SCOPE_TO_TYPE_MAPPING];
    if (mappedType) {
      console.log(`[getTokenType] Using first available mapping: "${scope}" -> "${mappedType}"`);
      return mappedType;
    }
  }
  ...
  console.log(`[getTokenType] No scope mappings found, using fallback type mapping: ${variable.resolvedType} -> ${fallbackType}`);
  return fallbackType;
}
...
class CSSExporter {
  ...
  async exportToCSS(options: CSSExportOptions): Promise<{
    ...
    try {
      ...
      console.log(`[CSSExporter] Built variable map with ${this.variableMap.size} variables`);
...
class JSONExporter {
  ...
  private async exportSingleJSON(
    ...
    console.log('exportSingleJSON called with:', {
    ...
    console.log('Processed tokens:', tokens, 'Token keys:', Object.keys(tokens));

    // Post-process for specific formats
    if (options.format === 'style-dictionary') {
      console.log('Applying Style Dictionary post-processing...');
      tokens = this.flattenForStyleDictionary(tokens);
    } else if (options.format === 'ant-design') {
      console.log('Applying Ant Design post-processing...');
  ...
  private processCollectionsToTokens(
    ...
    console.log('Processing collections to tokens:', collections.map(c => ({
      id: c.id,
      name: c.name,
      modes: c.modes?.length || 0
    })));

    for (const collection of collections) {
      const collectionVariables = variables.filter(v => v.variableCollectionId === collection.id);
      console.log(`Collection ${collection.name}: ${collectionVariables.length} variables`);

      const collectionTokens = this.processCollectionToTokens(collection, collectionVariables, options, variableMap);
      console.log(`Collection ${collection.name} generated tokens:`, Object.keys(collectionTokens));
    ...
    console.log('Final merged tokens keys:', Object.keys(tokens));
    return tokens;
  }
  ...
  private processCollectionToTokens(
    ...
    console.log(`Processing collection "${collection.name}" with ${variables.length} variables`);
    console.log('Variables in collection:', variables.map(v => ({
      name: v.name,
    ...
    for (const variable of variables) {
      ...
      if (options.types && options.types.length > 0) {
        const tokenType = FIGMA_TO_TOKEN_TYPE[variable.resolvedType];
        console.log(`Type filtering: variable ${variable.name} (${variable.resolvedType} → ${tokenType}), allowed types:`, options.types);
        ...
        if (!options.types.includes(variable.resolvedType) && !options.types.includes(tokenType)) {
          console.log(`Skipping variable ${variable.name} due to type filter`);
          continue;
        }
      }
  ...
  private generateTokenData(
    ...
    if (options.includeModes && collection.modes.length > 1) {
      console.log(`[Multi-Mode] Processing variable ${variable.name} with ${collection.modes.length} modes`);
      ...
      if (!hasAnyValue) {
        console.log(`Variable ${variable.name} has no values for any mode`);
        return null;
      }

      console.log(`[Multi-Mode] Generated multi-mode value for ${variable.name}:`, multiModeValue);
    ...
    if (options.modes && options.modes.length > 0) {
      ...
      if (foundMode) {
        ...
        console.log(`Variable ${variable.name} skipped: no selected modes match this collection`);
        return null;
      }
    }
    ...
    if (value === undefined) {
      console.log(`Variable ${variable.name} has no value for mode ${selectedMode.name}`);
      return null;
    }
    ...
    console.log(`Generating token for ${variable.name} with format: ${format}`, { format, tokenType });
    ...
    switch (format) {
      ...
      case 'style-dictionary':
        ...
        console.log(`[Style Dictionary] Generating token for ${variable.name}`);
        return {
          value: processedValue,
      ...
      case 'ant-design':
        ...
        console.log(`[Ant Design] Generating token for ${variable.name} with type: ${tokenType}`);
        const antToken: any = {
          value: processedValue
        };
  ...
  private flattenForStyleDictionary(tokens: any): any {
    ...
    console.log('Style Dictionary converted tokens:', Object.keys(converted));
  ...
  private organizeToCTI(tokens: any): any {
    ...
    traverse(tokens);
    console.log('[CTI Organization] Organized tokens into CTI structure:', Object.keys(organized));
    return organized;
  }
  ...
  private wrapForAntDesign(tokens: any): any {
    ...
    console.log('Ant Design wrapped tokens with keys:', Object.keys(antDesignTokens));
    console.log('Ant Design components:', Object.keys(components));
    return wrapped;
  }
...
class TextStyleExtractor {
  ...
  private async extractSingleTextStyle(style: TextStyle, options: any): Promise<ExtractedTextStyle> {
    ...
    console.log(`[TextStyleExtractor] Processing "${style.name}", preserveReferences: ${preserveReferences}`);
    console.log(`[TextStyleExtractor] boundVariables keys:`, Object.keys(boundVariables));
...
function initializePlugin(): void {
  ...
  console.log('[Plugin] Luckino Import Export Plugin initialized');
}
...
function validateColorValues(data: any) {
  console.log('[validateColorValues] Starting color validation...');
  ...
  validateObject(data);
  console.log('[validateColorValues] Color validation completed');
}
...
async function createVariablesFirstPass(data: any, collections: any, variables: any) {
  console.log('[createVariablesFirstPass] 🏗️ Creating variable structures...');
  ...
  console.log('[createVariablesFirstPass] ✓ All variable structures created');
}
...
async function createCollectionVariables(
  ...
  console.log(`[createCollectionVariables] Processing collection: ${collectionName}, currentPath: ${currentPath}, keys: ${Object.keys(obj).join(', ')}`);

  for (const [key, value] of Object.entries(obj)) {
    if (key.startsWith('$')) {
      console.log(`[createCollectionVariables] Skipping $ key: ${key}`);
      continue;
    }

    const path = currentPath ? `${currentPath}/${key}` : key;
    const fullPath = `${collectionName}/${path}`;
    console.log(`[createCollectionVariables] Processing key: ${key}, path: ${path}, fullPath: ${fullPath}`);
    ...
    if (value && typeof value === 'object') {
      ...
      if (tokenObj.$value !== undefined || tokenObj.value !== undefined) {
        ...
        try {
          ...
          console.log(`[createCollectionVariables] Creating/checking variable: ${path}, fullPath: ${fullPath}, collection: ${collectionName}`);

          let variable;
          if (existingVar && existingVar.resolvedType === figmaType) {
            console.log(`[createCollectionVariables] Reusing existing variable ${path}`);
            ...
            console.log(`[createCollectionVariables] Created new variable ${path}`);
          }
          ...
          console.log(`[createCollectionVariables] Stored variable as: ${fullPath}`);
        ...
        // This is a group - recurse
        console.log(`[createCollectionVariables] Processing group: ${path}`);
...
async function populateValuesSecondPassWithAliases(data: any, collections: any, variables: any, modes: string[]): Promise<number> {
  console.log('[populateValuesSecondPassWithAliases] 🎯 Assigning values to variables...');
  ...
  console.log(`[populateValuesSecondPassWithAliases] Processing ${aliasQueue.length} aliases...`);
  const processedAliases = await processAliasQueue(aliasQueue, collections, variables, modes);

  console.log('[populateValuesSecondPassWithAliases] ✓ All values assigned');
      ...
      if (tokenObj.$value !== undefined || tokenObj.value !== undefined) {
        ...
        try {
          ...
          if (typeof tokenValue === 'string' && isAliasValue(tokenValue)) {
            ...
            console.log(`[populateCollectionValuesWithAliases] Queued alias: ${path} = ${tokenValue}`);
            ...
            if (hasAliases) {
              ...
              console.log(`[populateCollectionValuesWithAliases] Queued multi-mode alias: ${path}`);
            } else {
              ...
              for (const [modeName, modeValue] of Object.entries(tokenValue)) {
                ...
                if (modeId) {
                  ...
                  console.log(`[populateCollectionValuesWithAliases] Set ${path} [${modeName}] = ${JSON.stringify(processedValue)}`);
                }
              }
            }
          } else {
            ...
            console.log(`[populateCollectionValuesWithAliases] Set ${path} = ${JSON.stringify(processedValue)}`);
          }
...
async function processAliasQueue(aliasQueue: any[], collections: any, variables: any, modes: string[]): Promise<number> {
  if (aliasQueue.length === 0) {
    console.log('[processAliasQueue] No aliases to process');
    return 0;
  }

  console.log(`[processAliasQueue] Processing ${aliasQueue.length} aliases...`);
    ...
    try {
      if (aliasItem.isMultiMode) {
        ...
        for (const [modeName, aliasValue] of Object.entries(aliasItem.modeAliases)) {
          ...
          if (resolvedAlias !== null) {
            ...
            if (modeId) {
              ...
              console.log(`[processAliasQueue] Resolved alias ${aliasItem.path} [${modeName}] -> VARIABLE_ALIAS(${resolvedAlias.id})`);
              processedCount++;
            }
          }
        }
        ...
        if (resolvedAlias !== null) {
          ...
          console.log(`[processAliasQueue] Resolved alias ${aliasItem.path} -> VARIABLE_ALIAS(${resolvedAlias.id})`);
          processedCount++;
        }
      }
    } catch (error) {
  ...
  console.log(`[processAliasQueue] ✓ Processed ${processedCount} aliases`);
...
async function resolveAliasValue(aliasStr: string, variables: any, currentCollection: string): Promise<any> {
  ...
  console.log(`[resolveAliasValue] Resolving alias: ${aliasPath} in collection: ${currentCollection}`);
  console.log(`[resolveAliasValue] Available variables:`, Object.keys(variables));
  ...
  for (const variation of variations) {
    console.log(`[resolveAliasValue] Trying variation: ${variation}`);
    ...
    if (targetVariable) {
      ...
      console.log(`[resolveAliasValue] Found target variable: ${variation} -> VARIABLE_ALIAS(${targetVariable.id})`);
      return aliasValue;
    } else {
      console.log(`[resolveAliasValue] No variable found for: ${variation}`);
    }
  }
          ...
          if (typeof tokenValue === 'object' && tokenValue.r === undefined) {
            ...
            for (const [modeName, modeValue] of Object.entries(tokenValue)) {
              ...
              if (modeId) {
                const processedValue = processSimpleValue(modeValue, variable.resolvedType);
                variable.setValueForMode(modeId, processedValue);
                console.log(`[populateCollectionValues] Set ${path} [${modeName}] = ${JSON.stringify(processedValue)}`);
              }
            }
          } else {
            ...
            console.log(`[populateCollectionValues] Set ${path} = ${JSON.stringify(processedValue)}`);
          }
...
function assignSimpleScopes(variable: any, tokenType: string, path: string): void {
  ...
  if (scopes.length > 0) {
    ...
    console.log(`[assignSimpleScopes] Applied corrected scopes to ${path} (${tokenType}): ${scopes.join(', ')}`);
  } else {
...
async function simpleImportVariables(jsonData: any): Promise<{
  ...
  try {
    console.log('[simpleImport] Starting simplified import process');

    let data = typeof jsonData === 'string' ? JSON.parse(jsonData) : jsonData;
    console.log('[simpleImport] Parsed JSON data');

    // Check if this is Token Studio format and convert if needed
    if (detectTokenStudioFormat(data)) {
      console.log('[simpleImport] Detected Token Studio format, converting...');
      data = convertTokenStudioFormat(data);
      console.log('[simpleImport] Converted to plugin format');
    }

    // Pre-validate colors to prevent crashes
    validateColorValues(data);
    console.log('[simpleImport] ✓ Color validation passed');
    ...
    for (const collectionName in data) {
      ...
      console.log(`[simpleImport] Detected modes for '${collectionName}':`, modes);
  ...
  try {
    ...
    if (collection) {
      ...
      if (missingModes.length > 0) {
        console.log(`[simpleImport] Adding missing modes to existing collection: ${missingModes.join(', ')}`);
        ...
        for (const modeName of missingModes) {
          if (collection.modes.length < LIMITS.MAX_MODES_PER_COLLECTION) {
            try {
              ...
              console.log(`[simpleImport] Added mode: ${modeName} (ID: ${modeId})`);
            } catch (error) {
    ...
    console.log(`[simpleImport] Creating new collection: ${name} (${collections.length + 1}/40)`);
    collection = figma.variables.createVariableCollection(name);
    ...
    for (let i = 1; i < modes.length && i < LIMITS.MAX_MODES_PER_COLLECTION; i++) {
      try {
        ...
        console.log(`[simpleImport] Added mode: ${modes[i]} to new collection`);
      } catch (error) {
    ...
    if (value.$value !== undefined || value.value !== undefined) {
      ...
      try {
        if (variable) {
          ...
          if (variable.resolvedType !== figmaType) {
            console.log(`[simpleImport] Type mismatch for ${currentPath}: ${variable.resolvedType} → ${figmaType}. Removing old variable.`);
            variable.remove();
        ...
        const processValue = (modeValue: any, modeId: string) => {
          ...
          if (isAliasMarker(processedModeValue)) {
            ...
            console.log(`[simpleImport] 🔗 Alias detected for ${currentPath}: {${processedModeValue.referencePath}}`);
            pendingAliases.push({
            ...
            console.log(`[simpleImport] ✓ Assigning concrete value for ${currentPath}`);
        ...
        if (isMultiMode) {
          ...
          console.log(`[simpleImport] Processing multi-mode value for ${currentPath}`);
          ...
          console.log(`[simpleImport] Processing single value for mode ${targetMode}`);
...
async function findVariableByPath(path: string): Promise<Variable | null> {
  ...
  if (localVariablesCache === null) {
    ...
    console.log(`[findVariableByPath] Loaded ${localVariablesCache.length} variables into cache`);
  }

  // First try: search by direct name
  let targetVar = localVariablesCache.find(v => v.name === figmaPath);
  if (targetVar) {
    console.log(`[findVariableByPath] Found variable by direct name: ${figmaPath}`);
    return targetVar;
  }
  ...
  if (pathParts.length > 1) {
    ...
    if (targetCollection) {
      ...
      if (crossCollectionVar) {
        console.log(`[findVariableByPath] Found cross-collection variable: ${collectionNameGuess}/${variableOnlyPath}`);
  ...
  for (const pending of pendingAliases) {
    ...
    if (targetVariable) {
      ...
      try {
        ...
        resolvedCount++;
        console.log(`[resolvePendingAliases] ✓ Resolved: ${variable.name} -> ${targetVariable.name}`);
      } catch (e) {
...
function parseColor(colorValue: any): any {
  console.log(`[parseColor] Input: ${JSON.stringify(colorValue)}, Type: ${typeof colorValue}`);

  // Handle different color formats
  if (typeof colorValue !== 'string') {
    console.log(`[parseColor] Non-string input, returning as-is:`, colorValue);
    return colorValue;
  }

  // Handle hex
  if (colorValue.startsWith('#')) {
    console.log(`[parseColor] Processing hex color: ${colorValue}`);
    const hex = colorValue.substring(1);
    console.log(`[parseColor] Hex without #: ${hex}`);
    ...
    if (hex.length === 3) {
      ...
      console.log(`[parseColor] Expanded 3-digit hex to: ${fullHex}`);
    }

    console.log(`[parseColor] Processing full hex: ${fullHex} (length: ${fullHex.length})`);
    ...
    if (fullHex.length === 8) {
      ...
      console.log(`[parseColor] Parsed 8-digit hex - R:${r}, G:${g}, B:${b}, A:${a}`);
    ...
    if (fullHex.length === 6) {
      ...
      console.log(`[parseColor] Hex components - R:${rHex}(${rInt}), G:${gHex}(${gInt}), B:${bHex}(${bInt})`);

      const r = rInt / 255;
      const g = gInt / 255;
      const b = bInt / 255;

      console.log(`[parseColor] Final RGB values - R:${r}, G:${g}, B:${b}`);
...
function extractModes(data: any): string[] {
  ...
  function findModes(obj: any): void {
    ...
    if (obj.$value && typeof obj.$value === 'object' && !obj.$value.r && !obj.$value.g && !obj.$value.b) {
      ...
      for (const modeName in obj.$value) {
        modes.add(modeName);
        console.log(`[extractModes] Found mode: ${modeName}`);
      }
    }
  ...
  if (!hasMultiModeValues && modes.size === 0) {
    modes.add("Default");
    console.log(`[extractModes] No multi-mode values found, using Default mode`);
  }
  ...
  console.log(`[extractModes] Final modes:`, result);
  return result;
}
...
function validateValueForVariableType(value: any, expectedType: string): any | AliasMarker {
  ...
  if (typeof value === 'string' && value.startsWith('{') && value.endsWith('}')) {
    ...
    if (referencePath.length > 0) {
      console.log(`[validateValueForVariableType] Alias reference detected:`, referencePath);
      return {
        __isAlias: true,
  ...
  switch (expectedType) {
    case 'FLOAT':
      if (typeof value === 'number') {
        console.log(`[validateValueForVariableType] FLOAT: Value is already a number`);
        return value;
      }
      ...
      if (typeof value === 'string') {
        ...
        if (!isNaN(numericValue)) {
          console.log(`[validateValueForVariableType] FLOAT: Parsed numeric value from string:`, numericValue);
          return numericValue;
        }
      }

      console.log(`[validateValueForVariableType] FLOAT: Converting to number, fallback to 0 if invalid`);
    ...
    case 'COLOR':
      ...
      try {
        ...
        console.log(`[validateValueForVariableType] COLOR: Parsed color:`, JSON.stringify(colorValue));
        return colorValue;
      } catch (error) {
    ...
    case 'STRING':
      if (typeof value === 'string') {
        console.log(`[validateValueForVariableType] STRING: Value is already a string`);
        return value;
      }
      ...
      if (typeof value === 'object' && value !== null) {
        try {
          ...
          console.log(`[validateValueForVariableType] STRING: Converted composite token object to JSON string:`, stringValue);
          return stringValue;
        } catch (e) {
      ...
      console.log(`[validateValueForVariableType] STRING: Converted to string:`, stringValue);
      return stringValue;

    case 'BOOLEAN':
      if (typeof value === 'boolean') {
        console.log(`[validateValueForVariableType] BOOLEAN: Value is already boolean`);
        return value;
      }
      ...
      console.log(`[validateValueForVariableType] BOOLEAN: Converted to boolean:`, boolValue);
      return boolValue;
    ...
    for (const setName of Object.keys(data)) {
      ...
      if (typeof tokenSet === 'object') {
        ...
        if (hasTokenStudioTokens) {
          console.log(`[detectTokenStudioFormat] Found Token Studio format in set: ${setName}`);
          return true;
        }
      }
    }
...
function convertTokenStudioFormat(data: any): any {
  console.log('[convertTokenStudioFormat] Converting Token Studio JSON to plugin format');
  ...
  console.log('[convertTokenStudioFormat] Processing token sets:', tokenSetOrder);

  // Process each token set
  for (const setName of tokenSetOrder) {
    if (!data[setName] || typeof data[setName] !== 'object') continue;

    console.log(`[convertTokenStudioFormat] Processing set: ${setName}`);
    converted[setName] = {};
  ...
  console.log('[convertTokenStudioFormat] Conversion complete');
  return converted;
}
  ...
  for (const [key, value] of Object.entries(source)) {
    if (value && typeof value === 'object') {
      ...
      if ((value as any).hasOwnProperty('value') && (value as any).hasOwnProperty('type')) {
        ...
        console.log(`[convertTokenStudioFormat] Converted token: ${key} (${(value as any).type} -> ${target[key].$type})`);
      } else {
...
function convertTokenStudioType(tokenStudioType: string): string {
  ...
  console.log(`[convertTokenStudioType] ${tokenStudioType} -> ${convertedType}`);
...
async function handleUIReady(): Promise<void> {
  try {
    const collections = await figma.variables.getLocalVariableCollectionsAsync();
    console.log(`[handleUIReady] Found ${collections.length} collections`);
    ...
    for (const collection of collections) {
      console.log(`[handleUIReady] Collection "${collection.name}" has ${collection.modes.length} modes, ${collection.variableIds.length} variables`);
      for (const mode of collection.modes) {
    ...
    console.log(`[handleUIReady] Total unique modes: ${modesArray.length}`);
    ...
    if (collections.length > 0 && collections.some(c => c.variableIds.length > 0)) {
      try {
        ...
        console.log('[handleUIReady] Starting W3C JSON export...');
        ...
        if (exportResult.success && exportResult.json) {
          ...
          console.log('[handleUIReady] ✅ W3C JSON exported successfully, length:', w3cJson.length);
          console.log('[handleUIReady] JSON preview:', w3cJson.substring(0, 200));
        } else {
      ...
      console.log('[handleUIReady] No collections or no variables, skipping export');
    }
...
async function handleExportTextStyles(msg: any): Promise<void> {
  try {
    ...
    console.log('[Text Styles] Extracted styles count:', result.styles.length);

    // Debug: Log all available properties from first style
    if (result.styles.length > 0) {
      const firstStyle: any = result.styles[0];
      console.log('[Text Styles] Available properties in first style:', Object.keys(firstStyle));
      console.log('[Text Styles] Sample values:', {
        name: firstStyle.name,
        textAlignHorizontal: firstStyle.textAlignHorizontal,
        textAutoResize: firstStyle.textAutoResize,
        hangingPunctuation: firstStyle.hangingPunctuation,
        hyperlink: firstStyle.hyperlink
      });
    }
    ...
    if (selectedProperties && selectedProperties.length > 0) {
      console.log('[Text Styles] Filtering with selected properties...');
      ...
      filteredStyles = result.styles.map((style, index) => {
        ...
        selectedProperties.forEach((prop: string) => {
          if (prop in style) {
            ...
            if (index === 0) {
              console.log(`[Text Styles] ✓ Property "${prop}":`, style[prop as keyof typeof style]);
            }
          } else if (index === 0) {
            console.log(`[Text Styles] ✗ Property "${prop}" NOT in style`);
          }
        });
      ...
      console.log('[Text Styles] First filtered style keys:', Object.keys(filteredStyles[0]));
    }
    ...
    console.log('[Text Styles] Export completed successfully');
    console.log('[Text Styles] ========== EXPORT END ==========');
