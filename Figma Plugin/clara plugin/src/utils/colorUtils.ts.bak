...
export function parseColor(colorValue: any): ParsedColor | any {
  ...
  console.log(`[parseColor] Unrecognized format, returning as-is:`, colorValue);
  return colorValue;
}
...
export function parseHexColor(hexColor: string): ParsedColor {
  console.log(`[parseHexColor] Processing hex color: ${hexColor}`);
  const hex = hexColor.substring(1);
  console.log(`[parseHexColor] Hex without #: ${hex}`);
  ...
  if (hex.length === 3) {
    ...
    console.log(`[parseHexColor] Expanded 3-digit hex to: ${fullHex}`);
  }

  console.log(`[parseHexColor] Processing full hex: ${fullHex} (length: ${fullHex.length})`);
  ...
  if (fullHex.length === 8) {
    ...
    console.log(`[parseHexColor] Parsed 8-digit hex - R:${r}, G:${g}, B:${b}, A:${a}`);
  ...
  if (fullHex.length === 6) {
    ...
    console.log(`[parseHexColor] Hex components - R:${rHex}(${rInt}), G:${gHex}(${gInt}), B:${bHex}(${bInt})`);

    const r = rInt / 255;
    const g = gInt / 255;
    const b = bInt / 255;

    console.log(`[parseHexColor] Final RGB values - R:${r}, G:${g}, B:${b}`);
          ...
          if (colorValue && typeof colorValue === 'string') {
            try {
              ...
              if (parsed !== colorValue) {
                console.log(`[validateColorValues] Fixed color at ${currentPath}: ${colorValue} -> ${JSON.stringify(parsed)}`);
                if ('$value' in value) {
                  value.$value = parsed;
                } else {
                  value.value = parsed;
