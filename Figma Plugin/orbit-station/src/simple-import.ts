...
export async function simpleImportVariables(
  jsonData: any,
  ...
  try {
    ...
    for (const collectionName in data) {
      ...
      // Process variables in this collection with preserveScopes option
      const varCount = await processCollectionSimple(collectionName, collectionData, collection, preserveScopes);
...
async function processCollectionSimple(
  ...
  for (const key in data) {
    ...
    if (value.$value !== undefined || value.value !== undefined) {
      ...
      // Create or reuse variable with proper scope preservation
      try {
        // STEP 1: Ensure we have the local variables cache
        if (localVariablesCache === null) {
          localVariablesCache = await figma.variables.getLocalVariablesAsync();
        }

        // STEP 2: Check for existing variable BEFORE creating
        let existingVar = localVariablesCache.find(v =>
          v.name === currentPath &&
          v.variableCollectionId === collectionInfo.collection.id
        );

        // STEP 3: Determine scopes behavior based on preserveScopes checkbox
        ...
        let scopesToApply: VariableScope[] | null = null;
        let shouldOverwriteScopes = preserveScopes; // Inverted logic!

        // If preserveScopes=false AND variable exists with scopes, preserve them
        if (!preserveScopes && existingVar && existingVar.scopes && existingVar.scopes.length > 0) {
          scopesToApply = [...existingVar.scopes];
          console.log(`[simpleImport] Preserving existing Figma scopes for ${currentPath}: ${scopesToApply.join(', ')}`);
        }
        // Otherwise (preserveScopes=true OR no existing scopes), will apply automatic scopes later

        // STEP 4: Create or reuse variable
        let variable: Variable;
        let isNewVariable = false;

        if (existingVar && existingVar.resolvedType === figmaType) {
          // Reuse existing variable (type matches)
          variable = existingVar;
          console.log(`[simpleImport] Reusing existing variable ${currentPath}`);
        } else {
          // Remove old variable if exists but type doesn't match
          if (existingVar) {
            console.log(`[simpleImport] Removing ${currentPath} (type changed from ${existingVar.resolvedType} to ${figmaType})`);
            existingVar.remove();
            // Update cache
            localVariablesCache = localVariablesCache.filter(v => v.id !== existingVar.id);
          }
          ...
          console.log(`[simpleImport] Created new variable ${currentPath}`);
          isNewVariable = true;

          // Add to cache
          localVariablesCache.push(variable);
        }

        // STEP 5: Set value (before applying scopes)
        ...
        // STEP 6: Apply scopes
        if (scopesToApply) {
          // Checkbox OFF + existing scopes found
          if (isNewVariable) {
            // Variable was recreated (type changed): REAPPLY saved scopes to new variable
            variable.scopes = scopesToApply;
            console.log(`[simpleImport] ✓ Reapplied preserved scopes to recreated variable ${currentPath}: ${scopesToApply.join(', ')}`);
          } else {
            // Variable was reused: scopes are already correct, DON'T touch them
            console.log(`[simpleImport] ✓ Scopes preserved (not modified) for ${currentPath}: ${scopesToApply.join(', ')}`);
          }
        } else {
          ...
          console.log(`[simpleImport] ✓ Applied automatic scopes for ${currentPath}`);
        }
...
// ================== FUTURE FEATURE: $EXTENSIONS SCOPES SUPPORT ==================
// Commented out for now - will be implemented in future version
// /**
//  * Extract scopes from token's $extensions field (W3C compliant)
//  * Expects: $extensions: { "com.figma.scopes": ["FRAME_FILL", "SHAPE_FILL"] }
//  */
// function extractScopesFromExtensions(tokenValue: any): VariableScope[] | null {
//   try {
//     const extensions = tokenValue?.$extensions;
//     if (!extensions || typeof extensions !== 'object') {
//       return null;
//     }
//
//     const figmaScopes = extensions['com.figma.scopes'];
//     if (!Array.isArray(figmaScopes) || figmaScopes.length === 0) {
//       return null;
//     }
//
//     // Validate all scopes are valid Figma scopes
//     const validScopes: VariableScope[] = [
//       'ALL_SCOPES', 'TEXT_CONTENT', 'CORNER_RADIUS', 'WIDTH_HEIGHT', 'GAP',
//       'ALL_FILLS', 'FRAME_FILL', 'SHAPE_FILL', 'TEXT_FILL',
//       'STROKE_COLOR', 'EFFECT_COLOR', 'STROKE_FLOAT', 'EFFECT_FLOAT', 'OPACITY',
//       'FONT_FAMILY', 'FONT_STYLE', 'FONT_WEIGHT', 'FONT_SIZE', 'LINE_HEIGHT',
//       'LETTER_SPACING', 'PARAGRAPH_SPACING', 'PARAGRAPH_INDENT'
//     ];
//
//     const filtered = figmaScopes.filter(scope =>
//       validScopes.includes(scope as VariableScope)
//     ) as VariableScope[];
//
//     if (filtered.length === 0) {
//       console.warn('[extractScopesFromExtensions] No valid Figma scopes found in $extensions');
//       return null;
//     }
//
//     console.log(`[extractScopesFromExtensions] Found ${filtered.length} valid scopes from $extensions: ${filtered.join(', ')}`);
//     return filtered;
//
//   } catch (error) {
//     console.warn('[extractScopesFromExtensions] Error parsing $extensions:', error);
//     return null;
//   }
// }
// ================== END FUTURE FEATURE ==================
