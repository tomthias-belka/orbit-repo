...
export class VariableExporter {
  ...
  private aliasResolver: AdvancedAliasResolver | null = null;
  private variableMap: Map<string, Variable> | null = null;
  ...
  async exportToJson(options: ExportOptions): Promise<{
    ...
    try {
      ...
      this.aliasResolver = new AdvancedAliasResolver(this.variableMap);
  ...
  private async processVariableValue(variable: Variable, value: any, options: ExportOptions): Promise<any> {
    ...
    if (value && typeof value === 'object' && value.type === 'VARIABLE_ALIAS' && value.id) {
      // Resolve alias to W3C reference format: "{path.to.token}"
      if (this.aliasResolver && this.variableMap) {
        const result = await this.aliasResolver.resolveAlias(value, {
          format: 'tokens',  // W3C Design Tokens format
        ...
        if (result.success && result.resolvedValue) {
          ...
          // Fallback: try to get variable name directly
          const aliasedVariable = this.variableMap.get(value.id);
          if (aliasedVariable) {
            return `{${aliasedVariable.name.replace(/\//g, '.')}}`;
          }
          // Ultimate fallback
          return `{UNRESOLVED_ALIAS:${value.id}}`;
        }
      }
    }

    // Process concrete values based on type
  ...
  private formatExportData(data: any, format?: string): any {
    // Orbit Station only supports W3C format
    // TOKEN_STUDIO and STYLE_DICTIONARY formats removed (not needed)
